...
export default async function DashboardOverviewPage() {
  const [stats, liveFeed, recentTx, volumeBuckets] = await Promise.all([
    getDashboardStats(),
    getLiveFeed(5),
    ...
    <div className="mx-auto max-w-[1920px] space-y-6 pb-10 md:pb-8">
      ...
      <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
        <KpiCard
          ...
          sub={stats.volume24h > 0 ? "Transactions réussies" : "Aucune transaction"}
          subVariant="trend"
        />
        <KpiCard
          title="Transactions 24h"
          value={String(stats.transactions24h)}
          sub={stats.transactions24h > 0 ? "Période glissante" : "Aucune"}
          subVariant="trend"
        />
        <KpiCard
          title="Taux de parse"
          ...
          sub={
            stats.transactions24h === 0
              ? "Sans activité sur 24 h"
              : "Réussites / total sur 24 h"
          }
          subVariant="badge"
        />
        <KpiCard
      ...
      {recentTx.length === 0 && (
        <section className="rounded-[var(--radius-mp-inner)] border border-mp-border bg-mp-surface p-8 text-center sf-card-shadow">
          <div className="mx-auto max-w-md">
            ...
            <p className="mt-2 text-sm text-mp-muted">
              Connectez une passerelle ou ingérez via l’API : les flux apparaîtront ici automatiquement.
            </p>
          </div>
        </section>
      )}
